...
function msdbPresentTable()
{
        ...
        msdbPerf("htbrowse.php", 162, (''));
        msdbStatPrepare();
        msdbPerf("htbrowse.php", 164, ('msdbStatPrepare'));

        if ( $dbMeta->ea ) {
                msdbPerf("htbrowse.php", 167, (''));
                msdbAction();
                msdbPerf("htbrowse.php", 169, ('msdbAction'));
        }

        msdbPerf("htbrowse.php", 172, (''));
        ...
        msdbPerf("htbrowse.php", 181, (''));

        $sql = "select * from $dbMeta->tname $dbMeta->whereEtc";
        if ( ($rows = msDbFetchRows($sql)) === null )
                msdbError("htbrowse.php".": ". 185 .": ".($sql));
        else
                msdbShowTable($rows);

        msdbPerf("htbrowse.php", 189, ('msdbShowTable'));

        msdbSend($rows);
        msdbPerf("htbrowse.php", 192, ('msdbSend'));
        msdbStatistics();
        msdbPerf("htbrowse.php", 194, ('msdbStatistics'));
        ...
        msdbChangeForm();
        msdbPerf("htbrowse.php", 202, ('msdbChangeForm'));
        msdbSearchForm();
        msdbPerf("htbrowse.php", 204, ('msdbSearchForm'));
}
...
function msdbHtbrowse()
{
        ...
        msdbPerf("htbrowse.php", 215, (''));

        msdbHoldMsgs();

        msdbPerf("htbrowse.php", 219, (''));
        $isH = msdbHead() ;
        msdbPerf("htbrowse.php", 221, ('msdbHead'));

        msdbHeader();

        msdbPerf("htbrowse.php", 225, ('msdbHeader'));

        msdbFlushMsgs();

        msdbPerf("htbrowse.php", 229, ('msdbFlushMsg'));

        if ( $isH || $dbMeta->noPerm ) {
                if ( $dbMeta->ea == 'msdbSelectDB' ) {
                        // before a switch DB
                        msdbListDBs();
                        msdbPerf("htbrowse.php", 235, ('msdbSelectDB'));
                } else if ( ! $dbMeta->tname || $dbMeta->noPerm ) {
                        msdbSelectTable();
                        msdbPerf("htbrowse.php", 238, ('msdbSelectTable'));
                }
                else
                        msdbPresentTable();
        } else {
        ...
        msdbPerf("htbrowse.php", 255, ('Done'));
