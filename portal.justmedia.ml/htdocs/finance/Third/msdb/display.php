...
function msdbTableLineContent($row)
{
        ...
        if ( $colNum != $rowColNum ) {
                msdbError("display.php".": ". 69 .": ".("msdbTableLineContent: row with $rowColNum columns, expected $colNum"));
                return(null);
        }
...
function msdbShowTableRow($row, $rowIndex)
{
        ...
        if ( $isKey ) {
                if ( isset($row[$dbMeta->pkIndex]) )
                        ...
                        msdb_r($row);
                        $sr = serialize($row);
                        msdbError("display.php".": ". 117 .": ".("index=$dbMeta->pkIndex but row is $sr"));
                        $pkValue = 'None' ;
                }
        }
...
function msdbIsSearchable($f)
{
        ...
        if ( ! isset($tyLimits[$ftype]) ) {
                msdbError("display.php".": ". 233 .": ".("msdbIsSearchable: No tyLimits entry for $ftype"));
                return(false);
        }
